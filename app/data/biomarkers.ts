...
export interface BiomarkerMeta {
  ...
  computed?: boolean
  // Tighter "optimization" targets shown as a nested band inside the lab reference range.
  // These are functional/longevity targets, generally narrower than the population reference
  // range a lab prints — a value can be in-range but outside optimal.
  optimalMin?: number
  optimalMax?: number
}
...
export const BIOMARKERS: Record<string, BiomarkerMeta> = {
  ...
  // The rest of the annual CHW panel (LabCorp reference ranges — Quest doesn't run these on the monthly draws).
  uric_acid: { label: 'Uric Acid', unit: 'mg/dL', category: 'metabolic', refMin: 3.8, refMax: 8.4, description: 'End product of purine metabolism, cleared by the kidneys. High levels drive gout and kidney stones and track with insulin resistance; fructose, alcohol, and dehydration push it up.' },
  phosphorus: { label: 'Phosphorus', unit: 'mg/dL', category: 'metabolic', refMin: 2.8, refMax: 4.1, description: 'Mineral partnered with calcium in bone and in cellular energy (ATP). Regulated by the kidneys, parathyroid hormone, and vitamin D — read it alongside calcium and eGFR.' },
  bun_creatinine_ratio: { label: 'BUN/Creatinine Ratio', unit: '', category: 'metabolic', refMin: 9, refMax: 20, description: 'BUN divided by creatinine. A high ratio points to dehydration or high protein intake rather than kidney damage; a low ratio can simply reflect high muscle mass raising creatinine.' },
  bilirubin_direct: { label: 'Bilirubin, Direct', unit: 'mg/dL', category: 'metabolic', refMax: 0.40, description: 'The conjugated fraction of bilirubin, already processed by the liver. Elevated direct bilirubin points to bile flow obstruction or liver cell damage rather than red cell breakdown.' },
  ggt: { label: 'GGT', unit: 'U/L', category: 'metabolic', refMax: 65, description: 'Gamma-Glutamyl Transferase — a liver and bile duct enzyme, the most sensitive marker for alcohol- and drug-induced liver stress. Rises with oral anabolics and cholestasis, and confirms that a high alk phos is hepatic rather than bone.' },
  ldh: { label: 'LDH', unit: 'U/L', category: 'metabolic', refMin: 121, refMax: 224, description: 'Lactate Dehydrogenase — an enzyme released from damaged cells anywhere in the body (muscle, liver, red cells). Non-specific; a hard workout before the draw can raise it.' },
  ...
  hdl: { label: 'HDL', unit: 'mg/dL', category: 'lipids', refMin: 40, optimalMin: 55, higherIsBetter: true, description: '"Good" cholesterol that removes excess LDL from arteries and returns it to the liver. Higher HDL is consistently associated with lower cardiovascular risk.' },
  ldl: { label: 'LDL', unit: 'mg/dL', category: 'lipids', refMax: 100, optimalMax: 70, description: '"Bad" cholesterol that deposits in artery walls and forms plaques. A key cardiovascular risk factor, though ApoB is now considered more accurate.' },
...
// Derived markers — computed from the raw values above at read time, never stored or extracted.
// Each `compute` returns null unless every input it needs is present in the draw, so a card
// only appears once the source markers have been recorded. Merged into BIOMARKERS below, so
...
// Fold derived values into a draw's markers. Won't clobber a value already on the draw, so a
// directly-reported marker always wins over a computed one of the same key.
export function computeMarkers(markers: Record<string, number | null>): Record<string, number | null> {
  const out = { ...markers }
  for (const [key, meta] of Object.entries(COMPUTED_MARKERS)) {
    if (out[key] == null) {
      const value = meta.compute(markers)
